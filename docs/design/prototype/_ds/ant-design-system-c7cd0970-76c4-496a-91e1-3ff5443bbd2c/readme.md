...
- \`tokens/fonts.css\` — font stacks + Google Fonts (Inter, Roboto). The bundled SF Pro Display Regular was removed from this repository (Apple system font, not redistributable); the stack falls back to the platform's native UI font.（字體堆疊；原先內嵌的 SF Pro Display Regular 已自本倉庫移除——那是 Apple 系統字體，不可散布——改由字體堆疊回退到各平台原生介面字體。）
...
- **SF Pro**: no longer embedded. The SF Pro Display Regular file was removed from this repository because it is Apple's system font and not redistributable. On macOS `-apple-system` still resolves to the real SF; elsewhere the stack falls back to Segoe UI / Roboto. SF Mono and PingFang SC likewise fall back to system fonts.
  （不再內嵌：SF Pro Display Regular 屬 Apple 系統字體、不可散布，已自本倉庫移除。macOS 上 `-apple-system` 仍解析為原生 SF；其他平台回退 Segoe UI／Roboto。SF Mono 與蘋方同樣回退系統字體。）
