...
const rows = computed(() => ai.localModels.map((m, i) => ({
  i,
  name: m.name,
  note: m.note,
  size: m.mb >= 1024 ? (m.mb / 1024).toFixed(1) + ' GB' : m.mb + ' MB',
  kind: m.kind,
  kindBg: m.kind === 'Embed' ? 'rgba(111,143,133,0.14)'
    : m.kind === 'Rerank' ? 'rgba(168,118,62,0.14)' : 'rgba(217,119,87,0.12)',
  kindColor: m.kind === 'Embed' ? '#4f6b62' : m.kind === 'Rerank' ? '#8a6428' : '#c0623f',
  actionLabel: m.installed ? 'Remove' : 'Download',
      ...
      <div class="card panel" style="gap: 8px">
        <div class="storage-head">
          ...
          <span class="card-sub" style="line-height: 18px">
            Tokenizer, embedding and rerank models run locally so categorising and search work offline.
          </span>
        </div>
        <div class="field" style="flex-shrink: 0">
          <label>Location</label>
          <input v-model="ai.modelDir" class="input input--sm input--code">
        </div>
        <div class="used">
          <span class="small dim-2">Used</span>
          <span style="font-size: 14px; font-weight: 600">{{ used }}</span>
        </div>
        <div class="foot-note">Local models are preferred; the cloud is only used for long-form summaries.</div>
      </div>

      <div class="card panel">
        <div class="card-title" style="flex-shrink: 0">Available models</div>
...
.storage-head { display: flex; flex-direction: column; gap: 2px; flex-shrink: 0; }
...
.model-note { font-size: 11px; color: var(--text-3); }
.action {
  width: 52px;
  flex-shrink: 0;
  text-align: right;
