...
const topics = computed(() => [{ name: 'All', count: dictionary.entries.length }]
  .concat(vocabCat.value.children.map((x) => ({
    name: x.name,
...
const rows = computed(() => {
  ...
  const tp = dictionary.topic || 'All'
  return dictionary.entries
    .filter((d) => (tp === 'All' || d.cat === tp)
...
const sources = [
  { id: 'ecdict', name: 'ECDICT', note: 'EN–ZH, ~770k entries with frequency tags' },
  { id: 'cedict', name: 'CC-CEDICT', note: 'ZH–EN, best for Chinese to English' },
  { id: 'wordnet', name: 'WordNet', note: 'Fills in missing examples' },
  { id: 'custom', name: 'Custom file', note: 'Your own word list' },
]

const mapFields: [key: string, label: string][] = [
  ['w', 'Word column'],
  ['zh', 'Meaning column'],
  ['ph', 'Phonetic column'],
  ['cat', 'Topic column'],
...
const dropTitle = computed(() => (dragOver.value
  ? 'Release to load'
  : dictionary.file ? 'Choose another file' : 'Drop a dictionary file here, or click to choose'))

const importBtn = computed(() => (dictionary.file ? 'Import ' + dictionary.file.rows + ' rows' : 'Import'))
...
<template>
  <div class="page">
    <div class="head">
      <div class="head-left">
        <input v-model="dictionary.query" class="input search" placeholder="Search word, meaning or topic">
        <span class="card-sub">Thematic dictionary · {{ dictionary.entries.length }} entries</span>
      </div>
      ...
      <div class="btn-slot" style="width: 152px" @click="dictionary.importOpen = true">
        <a-button type="primary">Import dictionary</a-button>
      </div>
    </div>
    ...
    <div class="grid" :style="{ gridTemplateColumns: libCols, gridTemplateRows: libRows }">
      <div class="card card--flush topics">
        ...
        <div class="scroll-y" style="padding: 4px 0">
          <div
            ...
            :key="t.name"
            class="topic"
            :class="{ 'is-on': (dictionary.topic || 'All') === t.name }"
            @click="dictionary.topic = t.name"
          >
            <span class="ell" style="flex: 1; min-width: 0">{{ t.name }}</span>
      ...
      <div class="card card--flush words">
        <div class="words-head">
          ...
          <span class="col-zh">Chinese</span>
        ...
        <div class="scroll-y">
          <div v-for="d in rows" :key="d.w" class="word-row">
            ...
            <span class="col-action drill" @click="drill(d.w, d.cat)">Drill</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div v-if="dictionary.importOpen" class="mask">
        <div class="modal" style="width: 600px">
          <div class="modal-head">
            <span style="font-size: 16px; font-weight: 600">Import dictionary</span>
          ...
          >
            ...
            <span style="font-size: 11px; color: var(--text-3)">
              .csv / .tsv / .json / .txt (ECDICT, CC-CEDICT exports)
            </span>
            <input
              ref="fileInput"
              type="file"
          ...
          <div v-if="dictionary.file" class="mapping">
            <div class="mapping-head">
              ...
              <span style="white-space: nowrap">Detected {{ dictionary.file.rows }} rows</span>
            </div>
            ...
            <div class="preview">
              <div class="preview-head">
                <span style="flex: 1; min-width: 0">Word</span>
                <span style="flex: 1; min-width: 0">Chinese</span>
                <span style="width: 76px; flex-shrink: 0">Topic</span>
              </div>
          ...
          <div class="dict-foot">
            <div class="dedupe" @click="dictionary.dedupe = !dictionary.dedupe">
              ...
              <span class="small dim-2">Keep mastery for duplicates</span>
            </div>
...
.search { width: 240px; max-width: 44vw; height: 32px; }

.grid { flex: 1; min-height: 0; display: grid; gap: 12px; }

.topics { display: flex; flex-direction: column; min-height: 0; overflow: hidden; padding: 0 12px 10px; }
.topics-head {
...
.topic.is-on { background: rgba(217, 119, 87, 0.1); color: var(--primary-active); }
.topic-count { font-size: 12px; opacity: 0.7; }

.words { min-height: 0; display: flex; flex-direction: column; }
.words-head {
  display: flex;
  gap: 12px;
  padding: 14px 0 12px;
...
.col-word { flex: 2 1 0; min-width: 0; }
.col-zh { flex: 1.4 1 0; min-width: 0; }
.col-mastery { width: 58px; flex-shrink: 0; }
.col-action { width: 52px; flex-shrink: 0; text-align: right; }
.word-main { display: flex; flex-direction: column; overflow: hidden; }
.word-pos { font-size: 11px; color: var(--text-3); margin-left: 6px; }
.word-ph { font-size: 11px; color: var(--text-3); }
.mastery { display: flex; flex-direction: column; align-items: flex-end; gap: 3px; }
.mastery-bar {
  width: 100%;
  height: 5px;
  border-radius: 100px;
...
.mastery-bar > i { display: block; height: 5px; border-radius: 100px; }
...
.source.is-on { border-color: var(--primary); background: rgba(217, 119, 87, 0.08); }
.source-name { font-size: 13px; font-weight: 500; }
.source.is-on .source-name { color: var(--primary-active); }
.source-note { font-size: 11px; color: var(--text-3); }

.drop--dict { padding: 20px; gap: 4px; }
...
.mapping-fields { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 8px; }
.mapping-label { font-size: 11px; color: var(--text-3); }
.mapping-select {
  height: 30px;
  border-radius: 6px;
  border: 1px solid var(--line-2);
...
.preview { border: 1px solid var(--line); border-radius: 6px; overflow: hidden; }
.preview-head {
  display: flex;
  gap: 10px;
  padding: 6px 10px;
...
.dedupe { display: flex; align-items: center; gap: 8px; cursor: pointer; }
</style>
