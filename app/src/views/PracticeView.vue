...
import { FINGER_NAME, ZH_REV } from '@/data/constants'
...
const kbHeight = computed(() => {
  const available = colH.value
    ? colH.value - (barH.value || 34) - 20
    : vh.value - 176
...
const typeFontSize = computed(() =>
  settings.fontSize === 'S' ? 'clamp(15px,2.1vh,19px)'
    : settings.fontSize === 'L' ? 'clamp(22px,3.2vh,30px)'
...
const chars = computed<CharCell[]>(() => {
  ...
  return t.split('').map((ch, i) => {
    ...
    if (typedCh !== undefined) {
      ...
      else { color = '#bc4b3c'; bg = 'rgba(188,75,60,0.12)'; deco = 'underline' }
    }
    const cur = i === session.typed.length
    return {
      ch: ch === ' ' ? ' ' : ch,
      color, bg, deco, cur, cell,
      anim: cur && idle ? (cell ? 'caretCell 1s steps(1) infinite' : 'caretPulse 1s steps(1) infinite') : 'none',
      shadow: cur
        ? (cell ? 'inset 0 -2px 0 0 #d97757' : 'inset 2px 0 0 0 #d97757')
        : (cell ? 'inset 0 -1px 0 0 #e6ded2' : 'none'),
    }
  })
})
...
const startBtn = computed(() =>
  session.count > 0 ? String(session.count) : session.startedAt ? 'Restart' : 'Start')
...
const prompt = computed(() => {
  ...
  return {
    ...
    ph: cur.ph,
    pos: cur.pos,
    progress: '#' + (at + 1) + ' / ' + l.length + ' 個　' + (half === 'zh' ? 'Chinese first' : 'then English'),
  }
})
...
const hintChar = computed(() => (session.nextRaw === ' ' ? 'Space' : session.nextRaw || ''))
const hintText = computed(() => {
  const f = session.nextFinger
  if (!f) return ''
  const key = session.layout === 'zhuyin' && session.nextRaw
    ? ' 按 ' + (ZH_REV[session.nextRaw] || '').toUpperCase() + ' 鍵'
    : ''
  return '，用' + FINGER_NAME[f] + key
})

/* ---------------------------------------------------------------- effects */

/** Keep the character under the caret vertically centred in the text card. */
watch(() => [session.typed, session.target, session.comp], async () => {
  await nextTick()
...
watch(() => session.typed, (v) => {
  const ime = imeRef.value
  if (ime && !session.composing && v === '' && ime.value !== '') ime.value = ''
})

watch(() => session.layout, () => nextTick(() => imeRef.value?.focus()))
    ...
    <div ref="barRef" class="toolbar">
      ...
      <div class="actions">
        <div class="btn-slot" style="width: 96px" @click="session.startRun()">
          <a-button>{{ startBtn }}</a-button>
        </div>
        <div class="btn-slot" style="width: 96px" @click="session.nextText()">
          <a-button type="primary">Next text</a-button>
        </div>
      </div>
    </div>
    ...
    <div class="text-card" :style="{ minHeight: textFloor + 'px' }">
      <div class="text-head">
        <div class="text-head-left">
          ...
          <span class="dim">{{ session.textMeta }}</span>
        </div>
        <span class="small dim">Typing starts the timer · Backspace to correct</span>
      </div>

      <div v-if="session.count > 0" class="countdown">
        <span class="countdown-value nums">{{ session.count }}</span>
        <span class="small dim">Get ready — timing starts at zero</span>
      </div>
      ...
      <div v-if="hintOpen" class="miss-hint">
        <span class="miss-count">3 misses</span>
        <span class="miss-text">Type <span class="miss-char">{{ hintChar }}</span>{{ hintText }}</span>
      </div>
    </div>
...
.pickers { display: flex; flex-direction: column; gap: 6px; min-width: 0; flex: 1; }
.picker-row { display: flex; align-items: center; gap: 8px; min-width: 0; }
.picker-label {
  font-size: 12px;
  color: var(--text-3);
  white-space: nowrap;
...
.seg--wrap { min-width: 0; flex-wrap: wrap; }
.seg-item--cat {
  display: flex;
  align-items: center;
  gap: 6px;
...
.topics { display: flex; gap: 6px; min-width: 0; flex-wrap: wrap; }
.actions { display: flex; gap: 8px; align-items: center; flex-shrink: 0; align-self: flex-start; }
...
.text-head-left { display: flex; gap: 8px; align-items: center; }
.text-head-left :deep(.ant-tag) { margin-inline-end: 0; }
...
.vocab-zh { font-size: 30px; line-height: 40px; font-weight: 600; color: var(--text); }
.vocab-meta { font-size: 13px; color: var(--text-3); }
...
.char--cell { margin: 0 3px 4px 0; padding: 0 2px; min-width: 1.35em; }
...
.miss-count { font-size: 12px; color: var(--primary-active); font-weight: 600; white-space: nowrap; }
.miss-text { font-size: 13px; color: var(--text-2); }
.miss-char {
  font-family: var(--font-mono);
  font-size: 14px;
  font-weight: 600;
