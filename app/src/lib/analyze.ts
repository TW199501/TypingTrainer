...
export function analyzeText(text: string): AnalysisResult | null {
  ...
  let subName = code
    ? (/SELECT|FROM|WHERE/i.test(src) ? 'SQL' : /using |namespace|public /.test(src) ? 'C#' : 'JavaScript')
    : category === 'Chinese' ? 'Daily talk' : 'Literature'
  ;(SUB_HINTS[category] || []).some(([re, name]) => {
    if (re.test(src)) { subName = name; return true }
    return false
  })
  ...
  return {
    ...
    chars: src.length,
    reason:
      'Language: ' + (han ? 'Chinese' : 'English') +
      ' · filed under ' + category + ' › ' + subName +
      ' · symbol density ' + Math.round(ratio * 100) + '%' +
