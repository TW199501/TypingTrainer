...
import { FING, FINGER_NAME, FMAP, ROWS, ROW_UNITS, ZHUYIN } from '@/data/constants'
    ...
    keys: row.map(([id, w, label]) => {
      ...
      return {
        ...
        shadow:
          ...
          ', inset 0 -3px 0 0 ' + (FING[finger] || 'transparent'),
      }
    }),
  })))

const legend = [
  { color: FING.pinkyL, label: 'Pinky' },
  { color: FING.ringL, label: 'Ring' },
  { color: FING.midL, label: 'Middle' },
  { color: FING.idxL, label: 'L index' },
  { color: FING.idxR, label: 'R index' },
]

const nextKeyLabel = computed(() =>
  session.nextRaw ? (session.nextRaw === ' ' ? 'Space' : session.nextRaw) : '—')
const nextFingerLabel = computed(() =>
  session.nextFinger ? FINGER_NAME[session.nextFinger] : '')
</script>
...
<template>
  <div class="keyboard" :style="{ height: props.height + 'px' }">
    <div v-if="props.showHint" class="kb-head">
      <span class="kb-title">
        Keyboard　Next key: <span class="kb-next">{{ nextKeyLabel }}</span>　{{ nextFingerLabel }}
      </span>
      <div class="legend" :style="{ opacity: props.legendVisible ? 1 : 0 }">
        <span v-for="f in legend" :key="f.label" class="legend-item">
          <span class="legend-swatch" :style="{ background: f.color }" />{{ f.label }}
        </span>
      </div>
    </div>
    <div class="kb-rows">
...
.kb-title { font-size: 14px; color: var(--text-2); }
.kb-next { color: var(--primary); font-weight: 600; }
...
.kb-row { display: flex; gap: 5px; width: 100%; flex: 1 1 0; min-height: 0; }
.kb-pad { min-width: 0; }
.key {
  height: 100%;
  display: flex;
...
.key-label { line-height: 1; }
.key-sub { font-size: 9px; line-height: 1; opacity: 0.5; margin-left: 3px; }
</style>
