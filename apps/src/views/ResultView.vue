...
const verdict = computed(() =>
  session.wpm >= 70 ? t('result.fast') : session.wpm >= 50 ? t('result.steady') : t('result.keepGoing'),
)
...
<template>
  <div class="page page--flow">
    <div class="score-card">
      <div class="dim">{{ t('result.yourScore') }}</div>
      <div class="score nums">{{ session.speed }}</div>
      <div class="score-sub">{{ t(`status.${session.speedUnitKey}`) }}　{{ verdict }}</div>
      <div class="metrics">
        <div class="metric">
          <div class="metric-label">{{ t('result.accuracy') }}</div>
          <div class="metric-value" :style="{ color: session.accColor }">{{ session.accuracy }}%</div>
        </div>
        <div class="metric">
          <div class="metric-label">{{ t('result.time') }}</div>
          <div class="metric-value">{{ elapsedLabel }}</div>
        </div>
        <div class="metric">
          <div class="metric-label">{{ t('result.errors') }}</div>
          <div class="metric-value" style="color: var(--error)">{{ session.errorCount }}</div>
        </div>
        <div class="metric">
          <div class="metric-label">{{ t('result.characters') }}</div>
